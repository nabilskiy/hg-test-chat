...
import { ArrowLeft, Send, Archive, ArrowDown, Paperclip, Shield } from 'lucide-react';
...
const GroupChatPage: React.FC = () => {
  ...
  const { apiCall, groupConversations, userId } = useApp();
  ...
  const conversation = groupConversations.find((c) => c.id === conversationId);
  const currentUserParticipant = conversation?.participants.find((p) => p.user_id === userId);
  const userRole = currentUserParticipant?.role || 'member';
  const canSendMessages = userRole !== 'member';
  ...
  const fetchMessages = async (showLoading = false, since?: string) => {
    ...
    try {
      ...
      const endpoint = since
        ? `/group-messages/${conversationId}?limit=50&since=${encodeURIComponent(since)}`
        : `/group-messages/${conversationId}?limit=50`;
  ...
  const markAsRead = async () => {
    if (!conversationId) return;
    try {
      await apiCall('POST', `/group-conversations/${conversationId}/read`, {});
    } catch (error) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!messageBody.trim() || !conversationId || !canSendMessages) return;
    ...
    try {
      ...
      await apiCall('POST', '/group-messages/send', body);
      setMessageBody('');
  ...
  const handleArchive = async () => {
    ...
    const confirmed = window.confirm('Are you sure you want to archive this group?');
    if (!confirmed) return;

    try {
      await apiCall('POST', `/group-conversations/${conversationId}/archive`, {});
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="shrink-0 bg-white border-b">
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="font-bold truncate">{conversation?.name || 'Group Chat'}</h1>
              {conversation && (
        ...
        {/* User Role Badge */}
        {conversation && currentUserParticipant && (
        ...
        {!canSendMessages ? (
          <div className="bg-orange-50 border border-orange-200 rounded-lg px-4 py-3 text-center">
            <p className="text-sm text-orange-700">
              Members cannot send messages (read-only)
            </p>
          </div>
        ) : (
