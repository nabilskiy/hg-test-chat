...
import { useApp, normalizeGroupConversations } from '../context/AppContext';
...
const GroupChatsPage: React.FC = () => {
  const { apiCall, groupConversations, setGroupConversations } = useApp();
  ...
  const fetchGroupConversations = async (showLoading = false) => {
    try {
      if (showLoading) setIsLoading(true);
      const data = await apiCall('GET', '/group-conversations');
      const rawConversations = Array.isArray(data) ? data : data.conversations || [];
      setGroupConversations(normalizeGroupConversations(rawConversations));
    } catch (error) {
  ...
  useEffect(() => {
    fetchGroupConversations(true);

    // Auto-poll every 25 seconds
    intervalRef.current = window.setInterval(() => {
      fetchGroupConversations(false);
    }, 25000);
  ...
  const handlePullRefresh = async () => {
    setIsPulling(true);
    await fetchGroupConversations(false);
    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="p-8 text-center">
            <p className="text-gray-500 mb-2">No group conversations yet</p>
            <p className="text-sm text-gray-400">Tap + to create a new group</p>
          </div>
        ) : (
